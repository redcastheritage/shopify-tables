...
(function ($) {
  ...
  if (window.localStorage.getItem("slim") !== null)
    $(".choose-slim")
      .val(window.localStorage.getItem("slim"))
      .trigger("change");
  ...
  if (window.localStorage.getItem("size") !== null)
    $(".choose-size")
      .val(window.localStorage.getItem("size"))
      .trigger("change");
  ...
  $(document).on("click", ".dupe-row", function () {
    ...
    $(this).closest("tr").after(clone).next().addClass("duped");
    generateJSON();
  });
  ...
  $(".table-builder .export-modal-html").click(function () {
    let json = generateJSON();
    exportModal.show();
    ...
    html += "</table>";
    $("#export-modal textarea").val(html);
    $("#export-modal textarea").focus().select();
  });
